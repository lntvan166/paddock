...
- **A name field, pre-filled and editable.** `agent.start` requires a name, and the route now refuses an absent one deliberately (see Task 7's fix) so this cannot ship as a silent default. Pre-fill it from the space's label, matching the convention §14.7 measured herdr itself using — `slug(workspace label)` — so the common case is one tap, not typing on a phone, and the operator can still override. **Do not default it to the harness kind**: every unnamed spawn would then be `claude`, `claude 2`, `claude 3`, which is a worse default than the one herdr already demonstrates using data the route's own tree read already has.
