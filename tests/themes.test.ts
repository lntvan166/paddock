...
test("the tour tokens are defined on bare :root, not only in a media query", () => {
  ...
  // direction, and an undefined colour is not a fallback — it is nothing.
  const base = baseTokens();
...
for (const t of THEMES.filter((x) => !NO_BLOCK.has(x.id))) {
  test(`tour callout stays AA on ${t.id}`, () => {
    const base = baseTokens();
    ...
    for (const fg of ["--tour-text", "--tour-accent"]) {
      const colour = own[fg] ?? base[fg]!;
      const r = ratio(colour, panel);
      expect(
        r,
        `${t.id}: ${fg} (${colour}) on --tour-panel (${panel}) is ${r.toFixed(2)}, below AA ${AA}`,
      ).toBeGreaterThanOrEqual(AA);
    }
  });
}
